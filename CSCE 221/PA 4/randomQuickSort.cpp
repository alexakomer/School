...
void quickSort(vector<T>& array, int leftIndex, int rightIndex)
{
  if (leftIndex >= rightIndex) // once indices cross, base case is reached and sort is finished
    return;
  int randomNum = rand() % (rightIndex - leftIndex) + leftIndex;
  int pivotKey = array[randomNum]; // select random element of each division as the pivot
...
int main ()
{
  ...
  cout << "random element pivot quickSort time elapsed: " << chrono::duration_cast<chrono::microseconds>(end-start).count() << " µs" <<  endl; //take delta time in microseconds
