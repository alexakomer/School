function requestInfo() { //pulls search hits and clicks from databse
...
function processInfo(response) { // analyze data and submit formatted arrays to relevant charts
  const hitKeys = [];
  const hitVals = [];
  const clickKeys = [];
  const clickVals = [];
...
function createPlot(keys, vals, type, chartNum) { // actual creation of each separate graph
...
function openTab(evt, tabName) { // handles switching between tabbed panes for graphs
  var i, x, tablinks;
...
function giveRecommendations(weighted, services) { // determines which services one should keep, add, or drop
  ...
  for (let key in services) {
    ...
    for (let i in weighted) {
      if (services[key] == weighted[i])
        included = true;
    }
    if(!included)
      rBad.push(services[key].toUpperCase());
  }
  
  let temp = weighted.slice(-3).reverse();
