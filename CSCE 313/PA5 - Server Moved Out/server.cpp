...
void process_file_request (TCPRequestChannel* rc, char* request)
{
	...
	// make sure that client is not requesting chunks that are too large
...
int main(int argc, char *argv[])
{
	...
	TCPRequestChannel * main_c = new TCPRequestChannel ("", port); // create server channel
	struct sockaddr_storage client_addr;
	socklen_t sin_s;

	while(true)
	{
		sin_s = sizeof(client_addr);
		int client_s = accept(main_c->getfd(), (struct sockaddr*)&client_addr, &sin_s);
		if(client_s == -1)
		{
			perror("accept");
			continue;
		...
		TCPRequestChannel * client_c = new TCPRequestChannel (client_s);
		thread t (handle_process_loop, client_c);
		t.detach();
	}
